function [WannierCenterCAR,WannierCenterWAVECAR,klist_l,WAVELOOPCAR] = WannierCenter(TBkitobj,optionsK,options)
% WANNIERCENTER Calculate Wannier centers along a 2D k-loop
%
% Syntax:
%   [WCC,WCCvec,klist] = WannierCenter(TBkitobj,optionsK)
%   [WCC,WCCvec,klist,WAVELOOP] = WannierCenter(...,options)
%
% Description:
%   Computes Wannier charge centers (WCCs) for a 2D k-loop parameterization
%   for different TBkit object types (Htrig, HK, HR).
%
% Input Arguments:
%   TBkitobj - Tight-binding object (Htrig, HK, or HR type)
%   optionsK - k-path parameters:
%       knum_int - Number of integration points
%       knum_evol - Number of evolution points
%       kstart - Starting k-point
%       kintegral - Integration direction
%       kevolution - Evolution direction
%   options - Additional options:
%       V - Perturbation matrix
%       Accuracy - Numerical accuracy
%       LWAVE - Flag to return wavefunctions
%       BAND_index - Band indices to include
%       script - Predefined k-path scripts
%
% Output Arguments:
%   WannierCenterCAR - Wannier center values
%   WannierCenterWAVECAR - Corresponding eigenvectors
%   klist_l - k-point list used
%   WAVELOOPCAR - Wavefunction data (if LWAVE=true)
%
% Example:
%   [wcc,vec,k] = WannierCenter(Hk,struct('knum_int',50));
arguments
    TBkitobj ;
...
switch class(TBkitobj)
    case {'Htrig','HK'}
        ...
        for i = 1:optionsK.knum_evol
            ...
            [WannierCenterCAR(:,i),WannierCenterWAVECAR(:,:,i)] = TBkit.WannierCenter1D(WAVECAR_loop_tmp);
        end
    case 'HR'
        for i = 1:optionsK.knum_evol
