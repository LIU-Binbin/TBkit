...
if options.WEIGHTCAR
    ...
    HSVCAR = TBkit.HSVCAR_gen(orb_list,options.ProjectionMethod,...
...
if strcmp(options.convention,'II')
    for ki =1:kn
        ...
        if options.WEIGHTCAR 
            [~,WEIGHTCAR(:,ki)] = TBkit.COLORCAR_gen(A,HSVCAR,signlist);
        end
        if OperMode
            [WEIGHTCAR(:,ki)] = TBkit.Observecar_gen(A,options.Oper);
        end
        if options.printmode
            pb.print(ki,kn,' ...');
        end
