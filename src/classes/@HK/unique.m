function [H_hk, Sublist, Unique_term] = unique(H_hk, seed, checklist, options)
%UNIQUE Identify and consolidate identical terms
%
% Syntax:
%   [H_simplified, Sublist, Unique] = unique(H_hk)
%   [...] = unique(H_hk, seed, checklist, options)
%
% Inputs:
%   H_hk - Original HK Hamiltonian
%   seed - Base symbol for new variables (default='eta')
%   checklist - Special coefficients to recognize (default common factors)
%   options - Control parameters:
%       Accuracy - Numerical tolerance (default=1e-6)
%       simplify - Enable simplification (default=true)
%
% Outputs:
%   H_hk - Simplified Hamiltonian
%   Sublist - Substitution rules
%   Unique_term - Identified unique terms
%
% Description:
%   Reduces Hamiltonian by:
%   1. Identifying identical coefficient terms
%   2. Substituting with symbolic variables
%   3. Recognizing special numerical relationships
%
% Example:
%   [H_simple, subs] = Hk.unique('xi'); % Simplify with xi variables
arguments
    H_hk HK;
    seed char = 'eta';
    checklist =sym([-1,sqrt(3),-sqrt(3),2,-2,3,-3,6,-6]);
    options.Accuracy = 1e-6;
    options.simplify logical=true;
end
HcoeL_tmp_1 = H_hk.HcoeL(:);
HcoeL_tmp = [real(HcoeL_tmp_1);imag(HcoeL_tmp_1)];
if  options.simplify
    HcoeL_tmp = TBkit.cleanVar(HcoeL_tmp,options.Accuracy);
end
[Unique_term,~,ic] = unique(HcoeL_tmp);
SymVar = sym(seed,[length(Unique_term),1],'real');
SymVar(Unique_term==sym(0)) = sym(0);
for coeff_for_check = checklist
    [Lia,Locb] = ismember(expand(Unique_term),expand(coeff_for_check*Unique_term));
    Equationlist = HR.isolateAll((SymVar(Lia) == coeff_for_check*SymVar(Locb(Locb>0)) ));
    SymVar = subs(SymVar,lhs(Equationlist),rhs(Equationlist));
end
Sublist = SymVar == Unique_term;
HcoeL_tmp_1 = SymVar(ic(1:end/2)) ...
    +1i*SymVar(ic(end/2+1:end));
H_hk.HcoeL = reshape(HcoeL_tmp_1,H_hk.Basis_num,H_hk.Basis_num,H_hk.Kinds);
end
