function [num_bands, Nmin, Nmax] = selectbands(EIGENCAR, Rm, mode)
    ...
    if nargin < 3
        mode = 'win_gen'; % Default mode is 'win_gen'
    end
    ...
    elseif strcmp(mode, 'dis_check')
        ...
        fprintf('Suggested range: (%d - %d)\n', anyk_max.n1, anyk_max.n2);
        fprintf('Energy window (min): %f\n', dis_min);
        fprintf('Energy window (max): %f\n', dis_max);
        fprintf('Frozen energy window (min): %f\n', foz_min);
        fprintf('Frozen energy window (max): %f\n', foz_max);
        
        % Alternative choices
        fprintf('Alternative choices: \n');
        fprintf('Energy window (min) < %f\n', dis_min);
        fprintf('Energy window (max) > %f\n', dis_max);
        fprintf('Frozen energy window (min) > %f\n', foz_min);
        fprintf('Frozen energy window (max) < %f\n', foz_max);
