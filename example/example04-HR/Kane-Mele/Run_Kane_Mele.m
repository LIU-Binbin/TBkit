...
Graphene = HR(2);
Graphene = Graphene.input_orb_struct('POSCAR','tbsk');
...
Kane_Mele_armchair_n = Kane_Mele_n.supercell_hr(Ns);
...
% slab band
% Efermi = 0.0;
Kane_Mele_n_slab.show('TwoD',1,'fast',1)
EIGENCAR_slab1 = Kane_Mele_n_slab.EIGENCAR_gen();
toc;
%
Kane_Mele_armchair_n_list = Kane_Mele_armchair_n.rewrite();
tic
Kane_Mele_n_slab = Kane_Mele_armchair_n_list.cut_piece(repeatnum,fin_dir,glue_edges,vacuum_mode);
% load KPOINTS
Kane_Mele_n_slab = Kane_Mele_n_slab < 'KPOINTS_slab';
% slab band
% Efermi = 0.0;
Kane_Mele_n_slab.show('TwoD',1,'fast',1)
EIGENCAR_slab2 = Kane_Mele_n_slab.EIGENCAR_gen();
toc;
% plot
bandplot({EIGENCAR_slab1,EIGENCAR_slab2},[-3,3],...
