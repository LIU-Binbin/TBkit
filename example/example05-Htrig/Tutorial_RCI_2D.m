...
clear;
Gamma_1_4 = sym(gamma_matrix(1,4));
Gamma_1_5 = sym(gamma_matrix(1,5));
Gamma_2_4 = sym(gamma_matrix(2,4));
Gamma_2_5 = sym(gamma_matrix(2,5));
%% useful tool
...
U = expm(-1i*pi*sym(gamma_matrix(2,5))/4);
%
...
%
RCI = RCI ...
    +Trig(sin(k_x) ,  gamma_matrix(1) )...
    +Trig(sin(k_y) ,  gamma_matrix(2))...
    +Trig(M-cos(k_x)-cos(k_y)   , gamma_matrix(3) )...
    ;
RCI_sym = sym(RCI);
...
C =m_1*gamma_matrix(4)+m_3*gamma_matrix(5)
m_x = m_0*cos(theta_0);
m_y = m_0*sin(theta_0) ;
% theta_x = theta_y;
